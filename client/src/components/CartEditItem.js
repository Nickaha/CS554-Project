...
function CartEditItem(props) {
  ...
  const updateItem = (id, new_bowl) => {
    if (
      !id ||
      !new_bowl.name ||
      typeof new_bowl.name !== "string" ||
      !new_bowl.base ||
      typeof new_bowl.base !== "string" ||
      !new_bowl.price ||
      typeof new_bowl.price !== "number" ||
      !new_bowl.protein ||
      !Array.isArray(new_bowl.protein) ||
      new_bowl.protein.length === 0 ||
      !new_bowl.topping ||
      !Array.isArray(new_bowl.topping) ||
      new_bowl.topping.length === 0 ||
      !new_bowl.sauce ||
      !Array.isArray(new_bowl.sauce) ||
      new_bowl.sauce.length === 0
    ) {
      console.log("Error: Mis-formatted order update not placed:");
      console.log(id, new_bowl);
    } else {
      dispatch(
        actions.editItemInCart(
          id,
          new_bowl.name,
          new_bowl.base,
          new_bowl.protein,
  ...
  if (props.bowl) {
    if (show) {
      if (isCustom(props.bowl.name)) {
        ...
        let protein_2 =
          props.bowl.protein.length >= 2 ? props.bowl.protein[1] : "chicken";
        let protein_3 =
          props.bowl.protein.length >= 3 ? props.bowl.protein[2] : "chicken";

        let topping_1 = props.bowl.topping[0];
        let topping_2 =
          props.bowl.topping.length >= 2
            ? props.bowl.topping[1]
            : "romaine lettuce";
        let topping_3 =
          props.bowl.topping.length >= 3
            ? props.bowl.topping[2]
            : "romaine lettuce";
        ...
        return (
          <form onSubmit={updateCustom}>
            ...
            <label>
              ...
              <br />
              <select
                defaultValue={numProtein.toString()}
                name="numProtein"
                id="numProtein"
                onChange={(e) => {
            ...
            <label>
              ...
              <br />
              <select
                defaultValue={props.bowl.sauce[0]}
                name="sauce"
                id="sauce"
              >
            ...
            <label>
              ...
              <br />
              <select
                defaultValue={numTopping.toString()}
                name="numTopping"
                id="numTopping"
                onChange={(e) => {
            ...
            <input type="submit" value="Edit Order" className="addtocart" />
          </form>
        );
      } else {
        return (
          <form onSubmit={updatePrebuilt}>
            ...
            <label>
              ...
              <br />
              <select
                defaultValue={props.bowl.name}
                name="bowl_name"
                id="bowl_name"
              >
                <option value="Gold Standard Chicken">
